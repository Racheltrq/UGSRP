...
def get_files(key):
    if key.type == 'maj':
        ...
        files = []
        files.append([str(key).replace(" ", "") + "1.midi", str(key).replace(" ", "") + "1.png"])
        files.append([str(key).replace(" ", "") + "2.midi", str(key).replace(" ", "") + "2.png"])
        files.append([str(key).replace(" ", "") + "3.midi", str(key).replace(" ", "") + "3.png"])
    return files
